...
class TestContainerInspectorCli(FileBasedTesting):
    ...
    def test_squash_single_layer(self):
        ...
        cli._container_inspector_squash(image_path=test_dir, extract_directory=target_dir)

        results = sorted([p.replace(target_dir, "") for p in fileutils.resource_iter(target_dir)])
        expected = ["/hello"]
