...
class TestDockerfile(FileBasedTesting):
    ...
    def test_normalized_layer_command(self):
        ...
        test_data = [
            ...
            ('#(nop) VOLUME ["/etc/elasticsearch"]',
             ('VOLUME', '["/etc/elasticsearch"]')),
            ('./usr/local/bin/run_rvm.sh', ('RUN', './usr/local/bin/run_rvm.sh')),
            ('', ('FROM', '')),
            (' ', ('FROM', '')),
            (None, ('FROM', '')),
            ('#(nop) ADD cacerts in /usr/java/openJDK-8-b132/jre/lib/security/cacerts',
             ('ADD', 'cacerts /usr/java/openJDK-8-b132/jre/lib/security/cacerts')),
            ('#(nop) CMD [/bin/sh -c supervisord -c /etc/supervisord.conf]',
             ('CMD', 'supervisord -c /etc/supervisord.conf')),
            ('#(nop)  ENV  CA_CERT_LOCATION=/etc/keys/ca.pem',
             ('ENV', 'CA_CERT_LOCATION=/etc/keys/ca.pem')),
            ('#(nop) EXPOSE map[7500/tcp:{}]', ('EXPOSE', 'map[7500/tcp:{}]')),
            ('#(nop) VOLUME ["/var/log", "/usr/local/pgsql/data"]',
             ('VOLUME', '["/var/log", "/usr/local/pgsql/data"]')),
            ('#(nop) WORKDIR /', ('WORKDIR', '/')),
        ]
